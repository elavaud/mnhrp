...
class CopyeditorAction extends Action {
	...
	function completeCopyedit($copyeditorSubmission, $send = false) {
		...
		if (!$email->isEnabled() || ($send && !$email->hasErrors())) {
			...
			ArticleLog::logEvent($copyeditorSubmission->getArticleId(), ARTICLE_LOG_COPYEDIT_INITIAL, ARTICLE_LOG_TYPE_COPYEDIT, $user->getId(), 'log.copyedit.initialEditComplete', Array('copyeditorName' => $user->getFullName(), 'articleId' => $copyeditorSubmission->getProposalId()));
